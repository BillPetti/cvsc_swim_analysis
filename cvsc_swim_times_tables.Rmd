...
swim_results_all <- swim_results_all %>%
  ...
  select(NAME, START, AGE_GROUP, age_start_season, SEX, stroke_name, DISTANCE, TYPE, swim_order, SCORE, change, cumulative_change)

names(swim_results_all) <- c("Swimmer", "Date", "Age Group", "Age", "Gender", "Stroke", "Distance", "Meet Type", "Swim Order", "Time", "Change", "Cumuative Change")

DT::datatable(swim_results_all, filter = "top", options = list(pageLength = 20, columnDefs = list(list(className = 'dt-center', targets = 3:11))))
```